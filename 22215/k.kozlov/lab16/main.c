...
Что такое терминал и псевдо-терминал? 
  ...
  Псевдотерминал, как следует из определения устройства, имтирует поведение обычного терминала. 
    Используется для удалённого подключения по ssh и другим протоколам, а также для терминальных сессий.
    Состоит псевдотерминал из двух устройств: ведущего /dev/pts/XX и ведомого /dev/pty/XX - подаваемые в ведущий терминал данные будут считываться ведомым и наоборот.
